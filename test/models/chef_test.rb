...
class ChefTest < ActiveSupport::TestCase
  def setup
    @chef = Chef.new(chefname: "Leo Chef", email: "chef@example.com")
  end
  ...
  test "email validation should accept valid addresses" do
    valid_addresses = %w[user@eee.com R_TDD-DS@eee.hello.org user@example.com 
                        first.last@ee.au laura+joe@monk.cm]
  ...
  test "email validation should reject invalid addresses" do
    invalid_addresses = %w[user@example,com user_at_eee.org user.name@example. 
                          eee@i_am_.com foo@ee+aa.com]
    invalid_addresses.each do |invalid_address|
      @chef.email = invalid_address
      assert_not @chef.valid?, "#{invalid_address.inspect} should be valid"
    end
  end
